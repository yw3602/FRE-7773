...
# COMET_API_KEY=dKDuNGGljTjCbs9tsRILwY81C MY_PROJECT_NAME=yw3602_team_final python3 dev_with_comet.py run
...
class MyRegressionFlow(FlowSpec):
    ...
    def valid_model(self):
        ...
        # create an experiment on comet for each branch
        exp_val = Experiment(
        ...
        exp_val.log_parameter("Model", self.model_name)
        exp_val.set_name(f"Validation with {self.model_name}")
    ...
    def train_best_model(self):
        ...
        self.best_MSE = mean_squared_error(self.df_y_test, y_test_pred)
        print("Best r2 is: ", self.best_r2)
        print("Best MSE is: ", self.best_MSE)

        # create an expreiment on comet
        exp_best = Experiment(
            api_key=os.environ['COMET_API_KEY'],
            project_name=os.environ['MY_PROJECT_NAME'],
            auto_param_logging=False
        ...
        exp_best.log_metrics(self.metrics_val)
        exp_best.log_parameter("Model", self.best_model[0])
        exp_best.set_name("Train best model")
